...
| No-LiDAR fallback | **Artifact path wired; model execution remains explicit** | `has_depth=False` still widens intervals and emits `video_only`; `load_capture()` can ingest a capture with no raw depth when a QC-approved dense artifact, calibration, and pose table are supplied. The frame contract rejects unapproved/malformed dense frames or falls back to same-index raw LiDAR, with all decisions recorded. Metric3D weights are never downloaded implicitly. |
